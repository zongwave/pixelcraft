...
4. **图像预处理契约：最容易翻车的一环**（真实事故复盘见第 02 章 §7）——
   ...
   - 建议两道闸并列：① 尺寸闸门（现有 `VideoToTensor.check_input`）；② **内容结构检查**
     （与该 checkpoint 训练帧的行亮度剖面结构比对，例如 PickNPlace/gr1 实测上下各 18.75% 纯黑）。
     第二条才抓得住"尺寸全绿但几何错了"这一类 bug——**比例必须实测，不能写死常数**
     （第 02 章 §7.3.1：同一批官方演示集里只有 PickNPlace 有黑边，其余黑边为 0）。
   - **别指望"上线看效果"来发现它**：第 02 章 §7.6 的离线 A/B 实测，同一帧只差这一步时
     token 数 / `seq_len` 逐项相同、backbone 特征余弦 0.9958、动作差异只有采样噪声的 1/536
     ——链路里**没有任何现成指标会报警**，只能在入口主动校验。
...
- 工程要点：并发图像解码、线程池推理、horizon 滚动执行、时间戳对齐、`torch.compile` 加速，
  以及**与训练一致的图像几何**（第 02 章 §7/§7.6：尺寸检查通过 ≠ 几何一致，非等比拉伸全程静默，
  真机上表现为定位偏移与末态失稳，而全链路指标无一报警）。
