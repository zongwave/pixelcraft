...
| `select_layer` | 16 | **12** | 从更靠前的层取特征——Qwen3-VL 深层更专门化，动作对齐点在更浅层 |
| `tune_top_llm_layers` | 4 | **0** | **回到"完全冻结 VLM"**——N1.5 哲学的回归：后训练不再微调语言塔，防遗忘优先 |
| `load_bf16` | true | false | 默认精度策略变化（对我们 NPU/融合算子的 dtype 编码有直接影响，见 §6） |
| state/action 维度 | 29 | **132** | 为全身控制（腿+手+灵巧手）扩容；也是第 11 章 G1 全身 embodiment 的前提 |
...
| DiT 层数 | 32 | **16** | **动作头减半**：语言塔更强后，头可以变轻——省下的算力全部让位给边缘部署延迟（Thor/Orin/DGX Spark 全支持） |
...
N1.7   Cosmos-Reason2-2B（Qwen3-VL 架构，gated）
       → 再回全冻（tune_top_llm_layers=0）、取特征层 16→12                （防遗忘回归 + 换开源生态）
```
...
### 主线② 动作头容量：16 → 32 → 16 的"两头挤"

DiT 加到 32 层（N1.6）换控制精度；一年后又砍回 16 层（N1.7），因为算力预算改由
VLM 侧的进步来补、并把省下的延迟预算全部投给边缘部署（ONNX/TRT/Thor）。
**动作头容量从来不是独立超参，它是"精度需求 × 部署延迟预算"的解。**
...
| 依赖管理 | pip + conda(py3.10) | 同左 | **uv** + py3.12 + torchcodec(仅 H.264 稳) |
...
## 6. 对本课程（N1.5 基准）与 groot_ops 的实际意义
...
3. **groot_ops/NPU 视角的三个直接抓手**：
   ...
   - DiT 层数（16↔32）、`select_layer`（16↔12）、bf16 默认翻转都会改变融合算子的
     shape/dtype 假设——统一 dtype 编码（0=fp16,1=bf16,2=fp32）时以 checkpoint config 为准；
...
| VLM 冻结策略 | 全冻 + 简化 adapter + LN | 删 adapter，解冻顶 4 层 | 全冻（tune=0），select_layer 16→12 |
| DiT 层数 | 16 | 32 | 16 |
| 动作空间 | 绝对关节/EEF | 相对动作（多数本体，默认） | 相对 EEF（人+机统一） |
| action_horizon | 16 | ckpt 50 / code 16 | code 40（G1 真机执行窗口默认 16） |
| state/action 维度 | 29 | 29 | 132 |
...
*整理时间：2026-09-16。N1.7 仍在快速迭代，数字以 GitHub main / HF 模型卡当时版本为准。*
