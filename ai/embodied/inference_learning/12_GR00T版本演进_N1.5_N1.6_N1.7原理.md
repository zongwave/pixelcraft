...
- **N1.7（相对 EEF 一统人机）**：维度上限扩到 132/132；相对性不再是模型 config
  ...
  零样本演示（`demo_data/droid_sample`）即靠该 tag 免微调。e2e MSE/延迟**待权重
  落盘后补测**（下载进行中，诚实标注）。
...
- **容量换延迟的实机代价**：N1.5→N1.6 同机同法，ZMQ 往返 0.596 s → 1.358 s
  （第 13 章 §5 总表）——主线②"DiT 16→32 层 + horizon 16→50"的容量提升，
  在推理侧是逐毫秒支付的。
...
| state/action 维度上限（ckpt） | 64 / 32 | 128 / 128 | 132 / 132（GR1 运行时 29 维，第 13 章实测） |
