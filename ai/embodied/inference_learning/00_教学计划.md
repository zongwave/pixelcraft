...
- **2026-09-24（八）**：**§6.7 跨去噪步 cross K/V 缓存从"预测"变成"实测"**——学生在板端
  `10.9.11.86`（`groot_ops@tmp_v02fix_ffi_guard` + `gr00t@tmp_v02fix_merge`，工作区未提交）实现并验证：
  kernel `adaln_qkv_kernel.ac` **+12/−4** 加第三态 `kv_from_nh1==2`（两个 `run_die` 各一处，签名不动 ⇒
  FFI/wheel ABI 零破坏）、host `action.py` **+32**（`GROOT_NPU_DIT_KV_CACHE` + 按 `vl_embs` 缓存 + 起始 reset）。
  实测：8 层 step1 计算 / step2-4 全 HIT；**ON vs OFF 逐位一致 `maxdiff=0.0`**；
  `action_pred` 0.999998、`left_hand` 0.9895（v0.2-fix 同口径）；**round 0.113 s(OFF) vs 0.114 s(ON) ⇒ 无端到端收益**。
  06 章 §6.7 附录新增**「落地实况」**（三坑兑现 + **落地时新暴露的第 4~6 坑**：`enc_key` 与 `_ditbufs`
  形状 key 的隐式代际耦合、`data_ptr` 复用只靠 reset 纪律（根治＝缓存项持强引用）、
  **数值对账对"失效逻辑"是盲区** ⇒ 需 HIT 计数 / 双 obs 守卫探针）+ 口径提醒（0.9895 与 0.99200 不可直接比）；
  09 章 §7 对账表第 1 行改判、§4.3 与图 9-4 红框改写为实测结论、Q2 标"已由实作作答"并附判卷要点。
  方法论两条：**融合会把优化吞进 FFI，可观测性必须跟着下沉一层**（隐形慢）；**"能省"≠"该省"，
  先量瓶颈再做常量提升**——省掉每层最肥的 GEMM（`M_kv≈296` vs `M_q=49`）e2e 也不动，
  这是"瓶颈不在算力"的可复现反证实验。
  **同日追加 trace 级对账**（新增 `tools/trace_kv_cache_diff.py`，ON/OFF 两份 LPU perf trace 六张表）：
  省下的算力精确落在唯一一个融合 kernel（`adaln_qkv_run_die` 单次 466 µs → HIT 185 µs，
  **−6.8 ms/step**，step/kernel/API 三处口径同数），而 **kernel 1273=1273、launch 1228=1228、
  sync 26=26、memcpy 46=46 全等** ⇒ "省不到发射"第一次被直接量到；设备忙比 70.2%→47.8%、
  空闲 +50 ms（增量集中在每个 DiT block 末尾 11→391 µs）⇒ 设备算完在等 host 发射。
  trace 还顺带兑现了坑 6：**kernel 单次时长的双峰就是现成的 HIT/MISS 计数探针**（每步 8 计算 + 24 HIT，
  两次采样都以 8 次全算开头 ⇒ reset 纪律生效），这是 `maxdiff=0.0` 看不见的信息。
  诚实边界写进正文：ON 那份采样 host 全局 ×1.5（补丁碰不到的 backbone 帧同样变慢）+ profiler 抬高绝对值
  ⇒ `Step_time` 的 +44 ms **不可归因**给缓存；单进程 2 step 单样本，要下定量结论需 N 次重复 A/B。
